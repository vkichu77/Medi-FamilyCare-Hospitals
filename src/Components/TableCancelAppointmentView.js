...
function TableCancelAppointmentView({ mystylebor, funcViewCancelAPPOINTMENT }) {
  ...
  useEffect(() => { 
    funcViewCancelAPPOINTMENT.current = getallcancelappointments;
    funcViewCancelAPPOINTMENT.current();
  }, [funcViewCancelAPPOINTMENT]);

  function getallcancelappointments() {
    fetch('/getallcancelappointments')
    .then(response => {
      if(response.status === 401) {          
        throw new Error("Error: Table AppointmentsDetails");
  ...
  {    
    ...
    setModalMessage(
      ...
      <p>APOINTMENT ID : <b>{aidFromaidString(tablerecs[id].AID)}</b></p>
      <p>APPOINTMENT DATE : <b>{tablerecs[id].CAPPDATE}</b></p>      
      <p>PATIENT ID : <b>{tablerecs[id].PATIENTID}</b></p>
      <p>PATIENT NAME : <b>{tablerecs[id].PNAME}</b></p>
      <p>SEX : <b>{tablerecs[id].SEX}</b></p>      
      <p>AGE : <b>{tablerecs[id].AGE}</b></p>      
          ...
          <Card className="shadow">
            <Card.Body>
              <div className="mb-3 mt-4">
                <h2 className="fw-bold mb-2 text-uppercase">View Records: Cancel Appointment Details</h2>
                <div className=" mt-5">
...
export default TableCancelAppointmentView;
