...
function TableStaffModify({ mystylebor, funcUpdate, funcView }) {
  ...
  const handleSubmit = (event) => {
    ...
    fetch("/updatepatient", {
      ...
      body: JSON.stringify({ PID, PNAME, ADDRESS, MOBILE, SEX, AGE }),
    }).then(response => {
      if(response.status === 401) {          
    ...
    .then(data => {
      ...
      if(data.OUTPUT.indexOf("ERR") !== -1) {    
        ...
        funcView.current();
        setModalMessage("Patient Details Successfully Updated");
        // alert(data.OUTPUT);
      }
      handleShow();
    })
          ...
          <Card className="shadow">
            <Card.Body>
              <div className="mb-3 mt-4">
                <h2 className="fw-bold mb-2 text-uppercase">Modify Table PatientDetails</h2>
                ...
                <Row className="mb-3">                    
                    ...
                    <Form.Label>PATIENTID</Form.Label>
                      <Form.Select placeholder="Select PATIENTID" as="select" disabled={buttonDisabled} required value={PID} onChange={(e) => setPID(parseInt(e.target.value))}>
