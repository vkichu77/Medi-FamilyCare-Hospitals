...
function TableStaffModify({ mystylebor, funcUpdateSTAFF, funcViewSTAFF }) {
  ...
  const dateFromDateString = (dateString) => {
    return moment(new Date(dateString)).format('YYYY-MM-DD 00:00:00');
  };
  ...
  useEffect(() => {      
    funcUpdateSTAFF.current = getallsids;
  ...
  const handleSubmit = (event) => {
    ...
    fetch("/updatestaff", {
      ...
      body: JSON.stringify({ SID, TYPE, TITLE, SNAME, QUALIFICATION, EXPERTISE, DPID, JOINDATE, ADDRESS, DESIGNATION, EMAIL, PASSWD, DOB, SEX }),
    }).then(response => {
      if(response.status === 401) {          
    ...
    .then(data => {
      ...
      if(data.OUTPUT.indexOf("ERR") !== -1) {    
        ...
        setModalMessage("Staff Details Successfully Updated");
        // alert(data.OUTPUT);
      }
      handleShow();
    })
  ...
  function handleClick() {
    ...
    .then(data => {      
      // var jd = dateDBtoForm(data.JOINDATE);
      // var dob = dateDBtoForm(data.DOB);
      // console.log(jd, dob);
          ...
          <Card className="shadow">
            <Card.Body>
              <div className="mb-3 mt-4">
                <h2 className="fw-bold mb-2 text-uppercase">Modify Table StaffDetails</h2>
                ...
                <Row className="mb-3">                    
                  ...
                  <Row className="mb-3">                    
                    <Form.Group
                      as={Col}
                      className="mb-3"
                      controlId="formQualificationMSTAFF"
                    >
                    ...
                    <Form.Group
                      as={Col}
                      className="mb-3"
                      controlId="forSpecializationMSTAFF"
                    >
                    <Form.Label className="text-center">
                        Specialization
                    </Form.Label>
                  ...
                  <Row className="mb-3">    
                    <Form.Group
                      as={Col}
                      className="mb-3"
                      controlId="formDepartmentMSTAFF"
                    >
                      <Form.Label className="text-center">
                        Department
                      </Form.Label>
                    ...
                    <Form.Group
                      as={Col}
                      className="mb-3"
                      controlId="formDesignationMSTAFF"
                    >
                  ...
                  <Row className="mb-3">
                    ...
                    <Form.Group
                      as={Col}
                      className="mb-3"
                      controlId="formBasicPasswordMSTAFF"
                    >
